...
class Entity_Extend:
    ...
    def entity_replace(self, te, ta, type):
        choice_ent = random.choices(
            self.type2entity[type], weights=self.type2weight[type], k=1
        )[0]
    ...
    def entities_extend(self, text, tag, ents):
        ...
        for ent in ents:
            ...
            (
                new_text[ent[0] + sign : ent[1] + 1 + sign],
                new_tag[ent[0] + sign : ent[1] + 1 + sign],
            ) = (te, ta)
...
if __name__ == "__main__":
    ...
    train_dataset = Nerdataset(
        train_text, train_label, tokenizer, max_len, tag2idx, enhance_data=True
    )
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)

    dev_dataset = Nerdataset(
        dev_text, dev_label, tokenizer, max_len, tag2idx, is_dev=True
    )
    ...
    if is_train:
        for e in range(epoch):
            ...
            if f1 > bestf1:
                bestf1 = f1
                print(
                    f'e={e},loss={loss_sum / ba:.5f} f1={f1:.5f} ---------------------->best'
                )
